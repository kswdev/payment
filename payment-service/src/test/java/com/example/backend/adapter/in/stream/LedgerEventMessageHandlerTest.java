...
@DisplayName("LedgerEventMessageHandler 단위 테스트")
class LedgerEventMessageHandlerTest {

    @Mock
    private PaymentCompleteUseCase paymentCompleteUseCase;

    @InjectMocks
    private LedgerEventMessageHandler handler;

    private Message<LedgerEventMessage> buildMessage(LedgerEventMessage eventMessage, ReceiverOffset receiverOffset) {
    ...
    class SuccessCase {
        ...
        @Test
        @DisplayName("원장 이벤트 수신 시 결제 완료 처리 후 오프셋을 커밋한다")
        void should_commit_offset_when_ledger_event_is_received() {
            // given
            ...
            Message<LedgerEventMessage> message = buildMessage(eventMessage, receiverOffset);
            ...
            Function<Flux<Message<LedgerEventMessage>>, Mono<Void>> ledgerFn = handler.ledger();

            // when
            StepVerifier.create(ledgerFn.apply(Flux.just(message)))
                    .verifyComplete();

            // then
            InOrder inOrder = inOrder(paymentCompleteUseCase, receiverOffset);

            inOrder.verify(paymentCompleteUseCase).completePayment(eventMessage);
            inOrder.verify(receiverOffset).commit();
        }

        @Test
        @DisplayName("여러 원장 이벤트 수신 시 모두 순차적으로 처리하고 각 오프셋을 커밋한다")
        void should_commit_offset_for_each_message_when_multiple_ledger_events_are_received() {

            // given
            LedgerEventMessage event1 = new LedgerEventMessage(LedgerEventMessage.Type.SUCCESS);
            LedgerEventMessage event2 = new LedgerEventMessage(LedgerEventMessage.Type.SUCCESS);
            ...
            List<Message<LedgerEventMessage>> messages = List.of(
            ...
            // when
            StepVerifier.create(ledgerFn.apply(Flux.fromIterable(messages)))
                    .verifyComplete();

            // then
            InOrder inOrder = inOrder(paymentCompleteUseCase, offset1, offset2);

            inOrder.verify(paymentCompleteUseCase).completePayment(event1);
            inOrder.verify(offset1).commit();
            inOrder.verify(paymentCompleteUseCase).completePayment(event2);
            inOrder.verify(offset2).commit();
        }
    }
